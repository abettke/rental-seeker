...
export class SeedExampleRentals1584817663777 implements MigrationInterface {
  ...
  async up(queryRunner: QueryRunner): Promise<void> {
    const realtors: User[] = await queryRunner.manager.getRepository(User).find({
      where: { role: Roles.REALTOR },
    });

    Array(20).fill(true).forEach(async (val, i) => {
      await queryRunner.manager.getRepository(Rental).save(new Rental({
